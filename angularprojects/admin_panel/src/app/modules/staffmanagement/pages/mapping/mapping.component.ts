...
export class MappingComponent implements AfterViewInit {
  ...
  getCompanyByStaffId() {
    this.apiService.fetchCompanyByUserid(this.staffId).subscribe((res: any) => {
      if (res.status == true) {
        this.dataSource = new MatTableDataSource(res.company);
  ...
  unMappingDialogue(details: any) {
    this.dialogRef = this.dialog.open(MappingDialog, {
      data: {
        flag: false,
    ...
    this.dialogRef.afterClosed().subscribe((res: any) => {
      if (res && res.flag == true) {
        let data = {
          staffid: details.user,
          companyid: details.company._id,
        };
        this.apiService
          .unmappingCompny(data.staffid, data.companyid)
          .subscribe((res) => {
            if (res.status == true) {
              this.snackBar.open("Company un-map successfully.", "Close", {
...
export class StaffmappingDialog implements OnInit {
  ...
  companyMap() {
    ...
    this.apiservice
      .companyMapping(this.data.staffid, body)
      .subscribe((res: any) => {
        if (res.status == true) {
          this.snackBar.open(res.message, "Close", {
            duration: this.durationInSeconds * 3000,
